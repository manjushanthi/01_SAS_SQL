...
PROC SQL;
	...
	CREATE TABLE WORK.D4_S0603_IMT_LKTHRU AS 

		SELECT 

			LK_THRG.PARENT_ISIN	AS C0010 	

			, CASE 
				WHEN LK_THRG.PARENT_ISIN IS NOT NULL THEN '1 - ISO/6166 for ISIN'
				ELSE 'NEED TO DERIVE'
			  END AS C0020


			, CASE 
				WHEN ((LK_THRG.PCT_OF_FUND*F.X_MARKET_VALUE)/100) < 0 THEN 'L - Liabilities'
				ELSE
					SUBSTR(TRIM(LK_THRG.CIC),3,1)||' - '||CIC_CAT.X_CIC_CATEGORY_NM  
			  END AS C0030
			...
			, CASE 
				WHEN LK_THRG.CURRENCY = 'GBP' 
					THEN '1 - Reporting currency' 
				ELSE 
					'2 - Foreign currency' 
			  END AS C0050	

			, SUM ( ( ( LK_THRG.PCT_OF_FUND * F.X_MARKET_VALUE ) / 100 ) ) AS C0060 FORMAT=21.2
    		...
    		, LK_THRG.CURRENCY 	AS CUR	     																													   			

		    , XIP.ENTITY_NAME 	AS ENTITY
