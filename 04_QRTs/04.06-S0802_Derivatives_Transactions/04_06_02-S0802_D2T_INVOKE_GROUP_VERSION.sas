/*   
	SL No		- 04.06.02
	Name 		- 02_D2T_Report_V1 GROUP - For Invoke.sql
	Description	- Invoke Version of QRT's - S0802 - D2T - Group reporting - New Logic
	Frequency 	- Quarterly
...
/* 
	...
	Extract both Part A and Part B for Group Invoke Extract
*/
...
CREATE TABLE WORK.S0802_D2T_INVOKE_GRP AS 

	SELECT 

	/****************************************** PART 1 START **********************************/ 		 

		 CASE WHEN ENTITY_LEI_STATUS = '1' THEN 'LEI/'
							 WHEN ENTITY_LEI_STATUS = '2' THEN 'SC/'
							 ELSE 'CHECK_X_EIOPA_CODE' END||ENTITY_LEI_CD AS C0020
		...
		, CASE  
				...
				ELSE 'CHECK X_EIOPA_CODE'
			END||CASE WHEN TRIM(ASSET_LIAB_DRVT_ID) LIKE 'Multiple%' THEN 'Multiple assets/liabilities' ELSE ASSET_LIAB_DRVT_ID END AS C0090
