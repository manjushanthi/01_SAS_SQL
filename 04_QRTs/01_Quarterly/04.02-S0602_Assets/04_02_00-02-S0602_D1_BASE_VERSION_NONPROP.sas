...
/*Create Temp table to load the extracted Data*/
	...
	CREATE TABLE WORK.S0602_D1_BASE AS 
		...
		SELECT 
			...
			, CASE 
				...
				WHEN PAM_USE.PAM_CUSIP IS NOT NULL AND UPCASE(ACC_GL_GROUP_DESC) <> 'CASH EQUIVALENTS' 
					...
					THEN TRIM(PAM_USE.ACC_SECURITY_DESC_LINE_1)||' - '||CASE WHEN B.ISSUE_CD IN ('XS0276103156','XS0731849831') THEN 'SENIOR CORP_BND' ELSE TRIM(COALESCE(PAM_USE.ACC_SECURITY_DESC_LINE_2,'')) END||' - '||TRIM(COALESCE(PAM_USE.ACC_SECURITY_DESC_LINE_3,''))
	...
	/****************EXTRA COLUMNS**********************************/	
			...
			, CASE
				...
				/*Added AFTER Q4-2020 RECON - START */	
				WHEN SUBSTR(CIC.ISSUE_CD,3,2) <> '85' AND  UPCASE(TRIM(NC.INFRA_CORP_FLAG)) = 'YES' AND CALCULATED SOLII_CREDIT_QUALITY  IN ('4', '5', '6') AND A.X_STRUCTURE LIKE '%NQ - O%' THEN '9'
				WHEN SUBSTR(CIC.ISSUE_CD,3,2) <> '85' AND  UPCASE(TRIM(NC.INFRA_CORP_FLAG)) = 'YES' AND CALCULATED SOLII_CREDIT_QUALITY  IN ('4', '5', '6') AND A.X_STRUCTURE LIKE '%SENIOR_O%' THEN '19'
				/*Added AFTER Q4-2020 RECON - END*/
		...
		WHERE 
			...
			AND (A.X_COLLATERAL_STATUS IS NULL OR A.X_COLLATERAL_STATUS =  '1')
