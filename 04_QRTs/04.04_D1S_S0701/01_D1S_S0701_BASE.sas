...
/*Create Temp table to load the extracted Data*/
	...
	CREATE TABLE WORK.S0701_D1S_BASE AS 
		...
		SELECT 
			...
			, CASE 
				 WHEN (F.SOURCE_SYSTEM_CD  = 'NONBLR' 
				    ...
				    THEN 
						CASE 
							WHEN SUBSTR(CIC.ISSUE_CD,3,1)  IN ('1', '2','5','6','8')  
								THEN PUT(F.HOLDINGS_NO,18.2) 
							ELSE PUT(A.PRESENT_VALUE_AMT,18.2) 
						 END
				ELSE 
					CASE 
						WHEN (F.SOURCE_SYSTEM_CD  <>  'NONBLR' 
	...
	/****************EXTRA COLUMNS**********************************/	
			...
			, INPUT( (CASE 
			  ...
			  , COALESCE( SEC_DETAIL.X_STRUCTURED_PRODUCT_TYPE , '-' ) AS C0070
						
			  , COALESCE( SEC_DETAIL.X_CAPITAL_PROTECTION , '-' ) AS C0080			
					
			  , COALESCE( SEC_DETAIL.X_UNDERLYING_TYPE , '-' ) AS C0090	
	
			  , COALESCE( SEC_DETAIL.X_CALL_OR_PUT , '-' ) AS C0100 	

			  , COALESCE( SEC_DETAIL.X_SYNTH_STRUCTURED_PRODUCT , '-' ) AS C0110 
					
			  , COALESCE( SEC_DETAIL.X_PREPAY_STRUCTURE_PRODUCT , '-' ) AS C0120 
			
  			  , COALESCE( SEC_DETAIL.X_COLLATERAL_CURR_BALANCE , . ) AS C0130 	
			 
			  , COALESCE( SEC_DETAIL.X_COLLATERAL_PORTFOLIO , '-' ) AS C0140 

			  , COALESCE( A.INITIAL_CONTRACT_RT , . ) AS C0150 
			
			  , COALESCE( SEC_DETAIL.X_VAR_ANNUAL_RETURN , '-') AS  C0160
					
			  , COALESCE( SEC_DETAIL.X_LOSS_GIVEN_DEFAULT , .) AS C0170
						
			  , COALESCE(  A.X_ATTACHMENT_POINT_PCT , .) AS C0180
						
			  , COALESCE(  A.X_DETACHMENT_POINT_PCT , .) AS C0190 			
												
			  , A.INTEREST_PAYMENT_TYPE_CD AS CPN_TYPE
		...
		LEFT JOIN				
						(				
						  SELECT 				
						  				X_STRUCTURED_PRODUCT_TYPE,
						  				X_CAPITAL_PROTECTION,
										X_COLLATERAL_CURR_BALANCE,
										X_CALL_OR_PUT,
										X_VAR_ANNUAL_RETURN,
										X_PREPAY_STRUCTURE_PRODUCT,
										X_UNDERLYING_TYPE,
										X_SYNTH_STRUCTURED_PRODUCT,
										X_COLLATERAL_PORTFOLIO,
										X_LOSS_GIVEN_DEFAULT,
										FINANCIAL_INSTRUMENT_RK
	
						  FROM			
							             test.SECURITIZATION_INSTRUMENT          			
						  WHERE				
									   DATEPART(VALID_FROM_DTTM) <= &AS_AT_MTH 																													
						  				AND DATEPART(VALID_TO_DTTM) > &AS_AT_MTH 		
						) SEC_DETAIL				
						ON A.FINANCIAL_INSTRUMENT_RK = SEC_DETAIL.FINANCIAL_INSTRUMENT_RK

																				
		WHERE 
			DATEPART(A.VALID_FROM_DTTM) <= &AS_AT_MTH                                                                                                                    
			AND DATEPART(A.VALID_TO_DTTM) > &AS_AT_MTH 
			AND SUBSTR(TRIM(CIC.ISSUE_CD),3,1) IN ('5','6')
