...
/* CHANGE PARAMETERS FOR SOLO ENTITY like - UKI , CIC and PIM - Partial Internal Model*/
PROC SQL ;
...
CREATE TABLE WORK.SF_FORECAST_SOLO AS 
	...
	SELECT
		...
		, SUM(CASE WHEN OWNER_OCC_PROP = 1 THEN (MKT_VALUE_DIV_1000) ELSE 0 END) AS OWNER_PROP
		...
		, SUM(CASE WHEN DATA_SOURCE <>  '03 - Lookthrough'  AND OTHER_SOLO = 1 THEN (MKT_VALUE_DIV_1000) ELSE 0 END) AS OTHER
		, SUM(CASE WHEN DATA_SOURCE =  '03 - Lookthrough'  AND OTHER_SOLO = 1 THEN (MKT_VALUE_DIV_1000) ELSE 0 END) AS LKTHRU_OTHER

		, SUM(
				CASE 
					WHEN  DATA_SOURCE IN ('01 - Blackrock - Without Lookthrough','02 - Non Blackrock - Without Lookthrough') AND SUBSTR(CIC,3,1)='4' THEN 0				
					WHEN I_T_CLS_GRP <> 'Exclude' THEN MKT_VALUE_DIV_1000 
					ELSE 0 
				END 
			) AS TOTAL
	...
	WHERE 
		TRIM(PORTF_NAME)IN ('U K Insurance Limited','U K Insurance Limited - ROI Branch')  	
		/*
		 -- UKI   IN ('U K Insurance Limited','U K Insurance Limited - ROI Branch' ) 		-- UKI
		 -- CIC   IN ('Churchill Insurance Company Limited') 					  			-- CIC
		 -- PIM   NOT IN ('Churchill Insurance Company Limited' , 'U K Insurance Limited') -- PIM Partial Internal Model
		*/
...
DATA SF_FORECAST_SOLO_FIN;
SET SF_FORECAST_SOLO	;
...
PROC MEANS NOPRINT DATA=SF_FORECAST_SOLO_FIN ;
...
INSERT INTO SF_FORECAST_SOLO_FIN
SELECT * FROM TEMP;

CREATE TABLE SF_FORECAST_SOLO_FINAL AS 
SELECT * FROM SF_FORECAST_SOLO_FIN;
QUIT;


PROC DATASETS LIBRARY=WORK NOLIST;
DELETE SF_FORECAST_SOLO TEMP SF_FORECAST_SOLO_FIN SUMMARY;
QUIT;
