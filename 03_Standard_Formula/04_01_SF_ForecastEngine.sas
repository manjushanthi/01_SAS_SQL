...
/*******************************************************************CALCULATED MEASURES REQUIRED FOR SPRD_RSK_BND START*************************************************/
		  ...
		  , CASE
			...
			, CASE
				...
				/*added in 28/01/2022*/
				/* for CIC = 21 and  BARCLAYS_FOUR_PILLAR_SUBINDUSTRY = 'Government Guaranteed' and EEA_MEMBER_FLAG <> 'Y' , GB IS NOT EEA POST BREXIT */
				WHEN ( SUBSTR(CIC,3,2)  IN ('11','14','13','19','21','23','24','25','28','29') AND BARCLAYS_FOUR_PILLAR_SUBINDUSTRY = 'Government Guaranteed' 
						AND X_EEA_MEMBER_FLAG <> 'Y' AND  CALCULATED ZERO_RATED = 0 AND CALCULATED CIC_11_15_NON_EU_CUR_MTCH =0 AND ( CALCULATED INFRA_CORP = 0  OR CALCULATED INFRA_CORP = -1) )  
					THEN 1
